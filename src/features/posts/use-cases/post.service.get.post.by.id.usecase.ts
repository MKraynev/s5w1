...
import { AvailableLikeStatus, LikeForPostRepoEntity } from "src/repo/likes/postLikes/entity/like.for.posts.repo.entity";
...
export class PostInfo{
    ...
    constructor(post: PostRepoEntity, userLikeStatus: AvailableLikeStatus, likes: number, dislikes: number, lastLikes: LikeForPostRepoEntity[]) {
        ...
        let newestLikes = lastLikes.length > 0? lastLikes.map(like => new LikeLiteInfo(like.createdAt.toISOString(), like.userId.toString(), like.user.login))
        :[]
...
export class PostServiceGetPostByIdUseCase implements ICommandHandler<PostServiceGetPostByIdCommand, PostInfo>{
    ...
    async execute(command: PostServiceGetPostByIdCommand): Promise<PostInfo> {
        ...
        const [post, userlike, statistic, lastLikes] = await Promise.all([
          ...
          if(!post)
        throw new NotFoundException('post doesnt ');
