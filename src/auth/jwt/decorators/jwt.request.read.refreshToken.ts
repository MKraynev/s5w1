...
export const ReadRefreshToken = createParamDecorator(
    async (data: any, ctx: ExecutionContext): Promise<JwtServiceUserRefreshTokenLoad> => {
        ...
        try {
            let decodedObject = await (new JwtService({ secret: JWT_SECRET })).verify(cookieToken) as JwtServiceUserRefreshTokenLoad;
