...
export class CommentServiceUpdateByIdCommand {
	constructor(public commentId:string, public userId: string, public commentData: CommentSetEntity) {}
}

@Injectable()
@CommandHandler(CommentServiceUpdateByIdCommand)
export class CommentServiceUpdateByIdUseCase implements ICommandHandler<CommentServiceUpdateByIdCommand, boolean>{
    ...
    async execute(command: CommentServiceUpdateByIdCommand) {
        ...
        if(!foundComment)
        throw new NotFoundException('Comment id doesnt exist')
