...
export const ReadAccessToken = createParamDecorator(
    async (data: TokenExpectation = TokenExpectation.Expected, ctx: ExecutionContext): Promise<JwtServiceUserAccessTokenLoad | undefined> => {

        let req = ctx.switchToHttp().getRequest() as (Request & { user: JwtServiceUserAccessTokenLoad });
